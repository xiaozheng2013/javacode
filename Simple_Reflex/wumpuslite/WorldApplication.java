...
class WorldApplication {
	...
	public static char[][][] generateRandomWumpusWorld(int seed, int size, boolean randomlyPlaceAgent) {
		...
		// Wumpus Generation
		/*
		 * Note here We can not ensure that the wumpus does not coincide with other elements
		 */
		x = randGen.nextInt(size);
		y = randGen.nextInt(size);
		...
		// Gold Generation 
		/*
		 * Note here We can not ensure that the Gold does not coincide with other elements
		 */
		x = randGen.nextInt(size);
		y = randGen.nextInt(size);
